...
static int	manage_outfile3(t_commands **com, t_f_str f_str, t_inc *i)
{
	int	end;
	...
	if ((*com)[i->i].sgl_cmd[i->j] != 0)
	{
		if (f_str.sp_chars[i->start] == '6')
			i->start += 1;
		(*com)[i->i].tab_outfile[i->n] = ft_strdup_s_to_e((*com)[i->i].sgl_cmd[i->j], i->start, end);
...
int	manage_outfile(t_commands **com, t_f_str f_str, t_inc *i, int *cpy_f)
{
	...
	if ((f_str.sp_chars[i->k] == '6' && f_str.sp_chars[i->k + 1] != '\0') || f_str.sp_chars[i->k] != '6')
	{
